...
This tutorial showcases a new feature of the Watson Discovery service -- Watson Discovery UI components. These components are tied directly to your Watson Discovery projects and can easily be plugged into your search application. The components are self-contained, and don't require any additional API calls to the Discovery service to retrieve data.

## Learning objectives

In this tutorial, you will learn how to navigate the Watson Discovery tooling to create and build your searchable data. This will include enhancing the enrichments applied to your data, and also building a custom panel to query and visualize your data.
...
Select the AirBnB reviews located [here](data).
...
As you can see, the default enrichments are `Parts of speech` and `Entities`. For our review data, we would also like to include `Keywords` and `Sentiment`.
...
Provide a meaningful label foro the UI component, then click `Apply`.
...
If you click on `View passage in document` for any of the reviews entries, you will drill-down into the review data. If you click on the `JSON` tab, you can see the actual result that is returned by Discovery.
...
Now that your search panel has been enhanced to show all the interesting facets of the data, let's build a application that will display that same panel.
...
From the root directory of your local repo, execute these commands to create the environment files. Note that they will be created in the `examples/discovery-search-app` directory.
...
Notice that all of the facets we added in the Discovery tooling shows up here.
...
The Discovery UI React components are imported in `src/App.js`.
...
In the above list, the main component is `DiscoverySearch`, and it requires that the Discovery client and project ID be passed in as parameters. This allows all of the details of each component to be self-contained, which means no additional programming is required to use them.
...
From the client-side perspective, the call to instantiate a Discovery service instance works because the authenticator is substituted with an actual token. This also means the client doesn't have to worry about  embedding the credentials in the client code or have them visible in the browser console. **CORS issue???**

## Summary

In this tutorial, we detailed how to navigate the Watson Discovery tooling to create a project and upload data into a collection. We then showed how to enrich the data and to build a custom search panel to query and visualize the data.
